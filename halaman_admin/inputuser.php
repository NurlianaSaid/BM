...
if (isset($_POST['submit'])) {
    $role = $_POST['role'];
    $id_kode = $_POST['id_kode']; // Bisa id_siswaa atau kode_guru
    $username = $_POST['username'];
    $password = password_hash($_POST['password'], PASSWORD_DEFAULT); // Hashing password untuk keamanan

    if ($role === 'siswa') {
        // Cek apakah siswa ada
        $checkSiswa = $conn->query("SELECT * FROM tb_siswa WHERE Id_siswaa = '$id_kode'");
        if ($checkSiswa->num_rows == 0) {
            echo "Siswa dengan ID $id_kode tidak ditemukan!";
            exit;
        }
        $id_siswaa = $id_kode;
        $kode_guru = null;  // Tidak memasukkan kode_guru
    } elseif ($role === 'guru') {
        // Cek apakah guru ada
        $checkGuru = $conn->query("SELECT * FROM tb_guru WHERE kode_guru = '$id_kode'");
        if ($checkGuru->num_rows == 0) {
            echo "Guru dengan kode $id_kode tidak ditemukan!";
            exit;
        }
        $id_siswaa = null;  // Jangan masukkan nilai untuk Id_siswaa jika role adalah guru
        $kode_guru = $id_kode;  // Masukkan kode_guru jika role adalah guru
    }

    // Query untuk memasukkan data ke tb_users
    $sql = "INSERT INTO tb_users (username, password, role, Id_siswaa, kode_guru) 
            VALUES ('$username', '$password', '$role', " . ($id_siswaa ? "'$id_siswaa'" : "NULL") . ", " . ($kode_guru ? "'$kode_guru'" : "NULL") . ")";

    // Eksekusi query
    if ($conn->query($sql) === TRUE) {
        echo "<script>
                alert('Data berhasil ditambahkan!');
                window.location.href = 'user.php'; // Redirect ke halaman daftar user
              </script>";
    } else {
...
<a class="nav-link1 dropdown-toggle" href="#" id="userDropdown" role="button"
    ...
    <div class="table-responsive">
        <form method="POST" action="inputuser.php">
            ...
            <div class="form-group">
                <label for="id_kode">ID/Kode : </label>
                <input type="text" id="id_kode" name="id_kode" placeholder="Masukkan ID/Kode" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="button" value="Batal" class="btn-cancel" onclick="window.location.href='user.php';">
            </div>
        </form>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        $('#id_kode').on('input', function() {
            var kode = $(this).val();  // Ambil nilai ID/Kode
            var role = $('#role').val();  // Ambil role (guru/siswa)

            // Cek jika kode ada
            if (kode.length > 0) {
                // Kirim request AJAX untuk mendapatkan nama berdasarkan kode dan role
                $.ajax({
                    url: 'get_nama.php',  // URL untuk memanggil file PHP
                    type: 'GET',
                    data: { kode: kode, role: role },  // Kirimkan data kode dan role ke server
                    success: function(response) {
                        // Isi field nama dengan response dari server
                        $('#nama').val(response);
                    }
                });
            } else {
                $('#nama').val('');  // Reset nama jika kode kosong
            }
        });
    });
</script>
